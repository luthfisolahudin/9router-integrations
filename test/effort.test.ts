...
test("maps every measured model to its highest observed route effort", () => {
	const entries = Object.entries(MEASURED_WIRE_EFFORT);
	// The live catalog must stay measured; an empty table would silently
	// disable every explicit effort. Exact membership is enforced by
	// `check:catalog` against the router, not here.
	assert.ok(entries.length > 0);
	for (const [id, effort] of entries) {
		assert.equal(measuredWireEffort(id), effort);
	}
